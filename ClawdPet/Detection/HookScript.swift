...
/// Escribe `clawdpet-hook` en `~/Library/Application Support/ClawdPet/`.
///
/// Se genera desde la app (en vez de copiarlo del bundle) para que siempre esté
/// actualizado y sea ejecutable, y para que exista aunque nunca instales el CLI.
/// El mismo contenido está en `Scripts/clawdpet-hook.sh` del repo.
enum HookScript {

    static var url: URL {
        ConfigStore.supportDirectory.appendingPathComponent("clawdpet-hook")
    }
    ...
    static func ensureInstalled() -> URL? {
        do {
            ...
            NSLog("[ClawdPet] No se pudo escribir el hook: \(error)")
            return nil
        }
    }
